...
def download_logo_in_memory(media_item):
    logo_url = f"{baseurl}/Items/{media_item['Id']}/Images/Primary?api_key={token}"
...
def download_latest_media(order_by, limit, media_type):
    ...
    # Filter out items based on excluded genres and tags
    filtered_items = []
                ...
                if response.status_code == 200:
                    ...
                    if media_type == 'Movie':
                        ...
                        if 'CommunityRating' in item:
                            ...
                            rating_text = ""
                        seasons_count = item['ChildCount']
                        seasons_text = "Season" if seasons_count == 1 else "Seasons"
                        info_text = f"{item['PremiereDate'][:4]}  •  {', '.join(item['Genres'])}  •  {seasons_count} {seasons_text}  •  {rating_text}"
